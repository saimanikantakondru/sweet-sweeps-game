...
// - [MODIFIED] Added Brightness, Contrast, Exposure controls

Shader "Spine/Skeleton Advanced" {
	Properties {
	...
	SubShader {
		...
		Pass {
			...
			// Applies Exposure -> Contrast -> Brightness in that order.
			// Alpha is preserved untouched.
			float3 ApplyColorGrading(float3 col) {
			...
			float4 frag (VertexOutput i) : SV_Target {
				...
				float4 result = fragTintedColor(texColor, _Black.rgb * i.vertexColor.a, i.vertexColor, _Color.a, _Black.a);

				// Apply color grading to RGB only, leave alpha (PMA) intact
				result.rgb = ApplyColorGrading(result.rgb);

				// Clamp to valid range — don't let contrast blow out to NaN
